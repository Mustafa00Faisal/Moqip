/* ============================================================================
   MOAQIB V7 — PRODUCTIVITY FOUNDATION 01
   ============================================================================
   This layer adds:
...
(() => {
    ...
    const PRODUCTIVITY_VERSION = 'v7-productivity-foundation-01';
    ...
    function applicationIsVisible() {
        const shell = document.getElementById('app-shell');
        const auth = document.getElementById('supabase-login-modal');
        return !!shell && !shell.classList.contains('hidden') && (!auth || auth.classList.contains('hidden'));
    }
    ...
    function installPerformanceWrappers() {
        ...
        if (typeof baseInitChart === 'function') {
            window.initChart = function deferredChartRender() {
                ...
                if (!applicationIsVisible() || !['home', 'analytics'].includes(view)) return;
        ...
        if (typeof baseSwitchTab === 'function') {
            window.switchTab = function optimizedSwitchTab(tabId, preserveHubMode = false) {
                ...
                if (typeof requestAnimationFrame === 'function') {
                    requestAnimationFrame(() => requestAnimationFrame(() => renderEnteredView(normalized)));
                } else {
                    setTimeout(() => renderEnteredView(normalized), 24);
                }
                return result;
            };
        }
    ...
    window.MOAQIB_PRODUCTIVITY_BOOTSTRAP = function productivityBootstrap(startV6) {
        ...
        idle(async () => {
            ...
            if (applicationIsVisible() && activeViewName() === 'home') window.initChart?.();
        }, 1300);
