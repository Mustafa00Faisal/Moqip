/*
 * ================================================================
 * نظام إدارة المعاملات المتقدم V7 — app.js
 * ================================================================
 *
 * ترتيب هذا الملف مهم:
...
// ==========================================================
        ...
        const APP_VERSION = '7.0.0-foundation.1';
...
// ==========================================
        ...
        function switchTab(tabId, preserveHubMode = false) {
            ...
            if (!target) return;
            const current = document.querySelector('.mq-view.active');
            const applyViewState = () => {
                document.querySelectorAll('.mq-view').forEach(el => el.classList.toggle('active', el === target));
            };
            let transitionHandled = false;
            try {
                if (typeof globalThis.MOAQIB_V7_NAVIGATE === 'function') {
                    transitionHandled = globalThis.MOAQIB_V7_NAVIGATE({ current, target, tabId, apply: applyViewState }) === true;
                }
            } catch (error) {
                console.warn('V7 navigation fallback:', error);
            }
            if (!transitionHandled) applyViewState();
        ...
        function toggleArchiveTab(type) {
            ...
            const targetPanel = mode === 'vault' ? vaultPanel : txPanel;
            const currentPanel = mode === 'vault' ? txPanel : vaultPanel;
            ...
            let transitionHandled = false;
            try {
                if (typeof globalThis.MOAQIB_V7_SWITCH_PANEL === 'function' && targetPanel) {
                    transitionHandled = globalThis.MOAQIB_V7_SWITCH_PANEL({
                        current: currentPanel,
                        target: targetPanel,
                        apply: applyArchiveState,
                        groupSelector: '#archive-tx-content,#archive-vault-content'
                    }) === true;
                }
            } catch (error) {
                console.warn('V7 archive transition fallback:', error);
            }
            if (!transitionHandled) applyArchiveState();
        ...
        function setSettingsPanel(panel = 'profile') {
            ...
            const currentPanel = document.querySelector('[data-settings-panel].active');
            const targetPanel = document.querySelector(`[data-settings-panel="${target}"]`);
            ...
            try {
                if (typeof globalThis.MOAQIB_V7_SWITCH_PANEL === 'function' && targetPanel) {
                    transitionHandled = globalThis.MOAQIB_V7_SWITCH_PANEL({
                        current: currentPanel,
                        target: targetPanel,
                        apply: applyPanelState,
                        groupSelector: '[data-settings-panel]'
                    }) === true;
                }
            } catch (error) {
                console.warn('V7 settings transition fallback:', error);
            }
            if (!transitionHandled) applyPanelState();
        ...
        function renderSettingsEntities() {
            ...
            // V7 performance foundation: aggregate relationships once. The V6
            // implementation rescanned every transaction for every card, which
            // became noticeably slower as a real office database grew.
            const companyCountByLawyer = new Map();
            const transactionCountByLawyer = new Map();
            const transactionCountByCompany = new Map();
            const documentCountByCompany = new Map();
            const increment = (map, key) => {
                const normalizedKey = String(key ?? '');
                map.set(normalizedKey, (map.get(normalizedKey) || 0) + 1);
            };
            db.companies.forEach(company => increment(companyCountByLawyer, company.lawyerId));
            getAllStoredTransactions().forEach(transaction => {
                increment(transactionCountByLawyer, transaction.lawyerId);
                increment(transactionCountByCompany, transaction.companyId);
            });
            db.vault.forEach(document => increment(documentCountByCompany, document.companyId));
        ...
        function openModal(id) {
            ...
            if (!modal) return;
            if (typeof globalThis.MOAQIB_V7_OPEN_MODAL === 'function' && globalThis.MOAQIB_V7_OPEN_MODAL(modal)) return;
            modal.style.display = 'flex';
        ...
        function closeModal(id) {
            const modal = document.getElementById(id);
            if (!modal) return;
            if (typeof globalThis.MOAQIB_V7_CLOSE_MODAL === 'function' && globalThis.MOAQIB_V7_CLOSE_MODAL(modal)) return;
            const content = modal.children[0];
        ...
        function renderAll() {
            ...
            db.transactions.forEach(transaction => {
                if (transaction.status === 'active') activeTxs.push(transaction);
                else if (transaction.status === 'stalled') stalledTxs.push(transaction);
            });
        ...
        if (typeof window.MOAQIB_V7_BOOTSTRAP === 'function') {
            window.MOAQIB_V7_BOOTSTRAP(startWithProductivity);
        } else {
            startWithProductivity();
        }
