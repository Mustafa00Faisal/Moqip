...
RUNTIME_JS = ["app.js", "productivity.js", "v7-runtime.js", "push-config.js", "service-worker.js"]
RUNTIME_CSS = ["ui-foundation.css", "productivity.css", "ui-rebuild.css", "v7-motion.css"]
...
migration_preview_path = ROOT / "v7" / "migration-preview.js"
migration_syntax = subprocess.run(
    ["node", "--check", str(migration_preview_path)], capture_output=True, text=True, check=False
)
check("JavaScript syntax: v7/migration-preview.js", migration_syntax.returncode == 0, migration_syntax.stderr.strip())
...
check("Service worker has a V7 cache", "moaqib-v7-foundation-01" in service_worker)
check(
...
v7_runtime_js = scripts["v7-runtime.js"]
v7_motion_css = (ROOT / "v7-motion.css").read_text(encoding="utf-8")
check("Application version is V7 foundation", "APP_VERSION = '7.0.0-foundation.1'" in app_js)
...
check(
    "V7 runtime loads before the business core",
    html.index('src="v7-runtime.js"') < html.index('src="app.js"'),
)
check(
    "V7 navigation has deterministic no-gap state",
    "MOAQIB_V7_NAVIGATE" in app_js
    and "setTimeout(() => target.classList.add('active'), 10)" not in app_js,
)
check(
    "V7 motion is adaptive and accessible",
    bool(re.search(r"prefers-reduced-motion\s*:\s*reduce", v7_motion_css))
    and "chooseMotionProfile" in v7_runtime_js
    and "data-mq-motion" in v7_motion_css,
)
check(
    "V7 mobile overlays avoid expensive blur",
    "@media(max-width:700px)" in v7_motion_css
    and "backdrop-filter:none!important" in v7_motion_css,
)
check(
    "V7 directory rendering aggregates relationships linearly",
    "const transactionCountByCompany = new Map()" in app_js
    and "getAllStoredTransactions().forEach" in app_js,
)
check(
    "V7 performance snapshot is locally observable",
    "getMoaqibV7PerformanceSnapshot" in v7_runtime_js
    and "PerformanceObserver" in v7_runtime_js,
)
schema_contract = json.loads((ROOT / "v7" / "schema-contract.json").read_text(encoding="utf-8"))
migration_preview_js = migration_preview_path.read_text(encoding="utf-8")
check(
    "V7 relational schema contract is explicitly non-live",
    schema_contract.get("status") == "design-only-not-live"
    and schema_contract.get("source", {}).get("snapshot_schema_version") == 6,
)
check(
    "V7 migration preview is read-only",
    "fetch(" not in migration_preview_js
    and "localStorage." not in migration_preview_js
    and ".from(" not in migration_preview_js,
)
for phase_id in [
...
hash_files = [
    "index.html", *RUNTIME_JS, *RUNTIME_CSS, "manifest.webmanifest", "SUPABASE_SECURITY_SETUP.sql",
    "v7/schema-contract.json", "v7/migration-preview.js",
]
hashes = {
